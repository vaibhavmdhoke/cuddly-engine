...
makeup_face_contouring = Category.create(name: 'Contouring', parent: makeup_face)
	10.times.each do |index|
		Product.create(name: "Product_#{makeup_face_contouring.name}_#{index}", category_id: makeup_face_contouring.id)
	end

makeup_face_primer = Category.create(name: 'Primer', parent: makeup_face)
	10.times.each do |index|
		Product.create(name: "Product_#{makeup_face_primer.name}_#{index}", category_id: makeup_face_primer.id)
	end

makeup_face_tools = Category.create(name: 'Tools', parent: makeup_face)
	10.times.each do |index|
		Product.create(name: "Product_#{makeup_face_tools.name}_#{index}", category_id: makeup_face_tools.id)
	end

makeup_face_foundation = Category.create(name: 'Foundation', parent: makeup_face)
	10.times.each do |index|
		Product.create(name: "Product_#{makeup_face_foundation.name}_#{index}", category_id: makeup_face_foundation.id)
	end

makeup_face_face_powder = Category.create(name: 'Face Powder', parent: makeup_face)
	10.times.each do |index|
		Product.create(name: "Product_#{makeup_face_face_powder.name}_#{index}", category_id: makeup_face_face_powder.id)
	end
